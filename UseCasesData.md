...
## #28 · Admin-edited config files 500 on prod: config/ is root-owned and git-tracked
**Problem:** pressing "Save" on /admin/prices (and latently /admin/settings, /admin/report-texts) on prod returned the 500 page. Local tests all passed.
**Cause:** the handlers wrote to `config/products.json` / `config/report_texts.json`. On prod the repo is a root-owned git clone and the web process runs as www-data, which owns only `data/` and `static/img/` -> `PermissionError: [Errno 13]`. Second latent trap: even with a chown, editing a git-tracked file on the server makes the next `git pull` in deploy.sh fail on local-changes conflicts.
**Solution:** overlay pattern — runtime edits live in gitignored `data/products.json` / `data/report_texts.json` (www-data-writable); `settings.get_products()` / `get_report_texts()` prefer the data/ file and fall back to the git-tracked `config/*.json` default. Admin saves write to data/ atomically (tmp + rename). Rule of thumb: **anything the admin UI writes must live under `data/`, never in the repo tree.**


## #29 · `deploy.sh` git-pulls itself: a change to deploy.sh applies only on the NEXT deploy
**Problem:** deploy V1.066 printed `WARNING: golosrisunka-free.service НЕ УСТАНОВЛЕН — бесплатные разборы не будут генерироваться` and skipped restarting the freemium worker, while the very next line of the same output said `free: active`. The unit was in fact installed, enabled and running. Worse, the shipped fix (V1.067) printed the same warning again — it looked like the fix didn't work.
**Cause, two layers.** (1) The check was `systemctl list-unit-files | grep -q '^golosrisunka-free\.service'`. Right after `systemctl restart golosrisunka-web golosrisunka-worker`, systemd was busy and the full unit-file listing came back without the unit, so the `else` branch fired: the worker was silently left on old code while the status line below still reported `active` (it uses `systemctl is-active`, a different call that worked). A "not installed" message that co-exists with "active" is the fingerprint. (2) `deploy.sh` runs `git pull` **inside itself**, so the copy bash is executing is still the pre-pull one — a change to deploy.sh takes effect only on the FOLLOWING run (and bash re-reading a script file that changed mid-run is undefined behaviour in general).
**Solution:** test the single unit instead of parsing a listing — `if systemctl cat golosrisunka-free.service >/dev/null 2>&1; then` (verified both directions on the box: real unit → 0, bogus unit → non-zero). And **after shipping any deploy.sh change, run `./deploy.sh` a second time** and confirm the new behaviour; the first run proves nothing about the new script. Same rule applies to `restart.sh` and anything else the script pulls over itself.


## #30 · 90% of "human" visits were vulnerability scanners: a visit row was created on ANY first request
**Problem (audit 02.09.2026):** `web_visits` showed 2 660 human visits in 20 days; only 247 browsers ever ran `track.js`. Top "direct" entry paths were `/.env`, `/.git/config`, `/wp-login.php`; one scanner created 1 259 "visitors" from Andorra in an hour and spoofed the site as referer (→ channel "internal"). Funnel denominators were inflated ~2.5×, «Действия» flooded with 7 592 error_404.
**Cause:** `track.after_request` → `_touch_visit()` inserted a visit for every request carrying a fresh `gr_s` cookie — including 404s, robots.txt and beacons — and `parse_device()` treated any "Mozilla" UA as human.
**Solution:** a NEW visit starts only with a successful HTML page (`page or not new_visit`); non-page first requests get neither a row nor a cookie. Empty UA = bot. 404s on scanner paths (`track.is_scanner_path`) are not tracked. Funnels count only visits whose browser sent `screen_w` (`admin_funnels.REAL_VISIT`). Rule: never count a request as a visit until the client proves it can run JS.

## #31 · Sequential order ids + unauthenticated success page = customer email leak
**Problem:** `GET /order/success/<id>` (and `/pay/<id>`, payment status/create) rendered «Пришлём PDF на <email>» for any existing id.
**Cause:** the pages were built for the buyer's own redirect and never checked who is asking; robots `Disallow` hid them from crawlers only.
**Solution:** `orders.access_token` (random, per order) stored in cookie `gr_o<id>` of the ordering browser (set at creation and on bank return); the logged-in owner also passes. No access → 404 for unpaid, redirect to `/login` for paid. Rule: any URL containing a sequential id must prove ownership, not obscurity.
