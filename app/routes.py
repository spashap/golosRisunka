...
def _schema_jsonld() -> str:
    ...
    org = {
        ...
        "description": "Анализ детского рисунка по фото: персональный отчёт о том, что "
                       "рисунки расскажут о ребёнке — сильные стороны, интересы и навыки. "
                       "На основе методик Пиаже, Ловенфельда, Выготского. Без диагнозов.",
        "knowsLanguage": "ru",
    }
    website = {
        "@context": "https://schema.org",
    ...
    product = {
        ...
        "description": "Тёплый персональный PDF-отчёт по рисункам ребёнка: сильные стороны, "
                       "как он работает с цветом, формой и деталями, и что можно поддержать "
                       "дома. 7 направлений развития с оценками и понятными объяснениями. "
                       "Образовательное наблюдение, без диагнозов.",
...
FAQ_ITEMS = [
    # Любопытство-первым: что родитель узнает (репозиционирование 15.06).
    ("Что интересного я могу узнать о ребёнке из отчёта?",
     "Вы увидите сильные стороны ребёнка, что повторяется из рисунка в рисунок, что привлекает "
     "его внимание, какие творческие привычки и навыки сейчас формируются. Всё это — наблюдения "
     "по видимым деталям рисунка: без диагнозов и скрытых смыслов."),
    # Конверсионный набор (spec B13) — снятие возражений.
    ("Какой возраст ребёнка подходит?",
     "От 3 до 12 лет. Отчёт учитывает возрастной этап развития рисунка: то, что "
     "типично для трёх лет, оценивается иначе, чем для девяти."),
    ("Что нужно прислать?",
    ...
    ("Это психологическая диагностика? Это не диагноз?",
     ...
     "в рисунке: линия, форма, детали, композиция, сюжет, контроль движений и возрастной "
     "этап. Мы не делаем выводов о психологическом состоянии ребёнка по одному рисунку."),
    ("Что означают оценки 6/10, 8/10 или 9/10?",
     "Оценки относятся к конкретному рисунку или серии рисунков, а не к ребёнку в целом. "
     "Средний балл не означает проблему. Иногда направление просто меньше видно в выбранном "
     "сюжете — например, если на рисунке один объект и нет персонажей."),
    ("Как понять, что отчёт не общий шаблон?",
     "В отчёте выводы должны быть привязаны к конкретным деталям рисунка: линии, композиции, "
     "цветовым решениям, деталям, сюжету, материалам и возрасту ребёнка. Именно поэтому мы "
     "просим не только фото, но и короткий контекст."),
    ("А если рисунок меня тревожит?",
     "Отчёт поможет спокойно посмотреть на видимые признаки рисунка и отделить наблюдения "
     "от популярных мифов. Но если вас серьёзно беспокоит состояние, поведение или "
     "безопасность ребёнка, лучше обратиться к профильному специалисту — отчёт не заменяет "
     "консультацию."),
