...
ACTIONS: list[tuple[str, str, str, bool, bool]] = [
    ...
    # Выбор тревоги карточкой = сегмент рекламы: по нему видно, на какой креатив
    # откликается аудитория, ещё до того как человек дошёл до анкеты.
    ("freecheck_card_black", "Проверка: карточка «тёмные цвета»", "проверка", True, False),
    ("freecheck_card_alone", "Проверка: карточка «рисует себя отдельно»", "проверка", True, False),
    ("freecheck_card_monsters", "Проверка: карточка «монстры, оружие»", "проверка", True, False),
    ("freecheck_card_repeat", "Проверка: карточка «одно и то же»", "проверка", True, False),
    ("freecheck_card_faceless", "Проверка: карточка «без лиц и рук»", "проверка", True, False),
    ("freecheck_card_pressure", "Проверка: карточка «давит, зачёркивает»", "проверка", True, False),
    ("freecheck_card_stopped", "Проверка: карточка «перестал рисовать»", "проверка", True, False),
    ("freecheck_card_neutral", "Проверка: карточка «просто интересно»", "проверка", True, False),
    ("free_concern_preset", "Фремиум: тревога пришла ссылкой с посадочной", "фремиум", True, False),
