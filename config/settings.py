...
# Таймаут одного запроса к Gemini (мс). Без него зависший вызов (упавший прокси/сеть)
# блокирует воркер НАВСЕГДА — заказ висит в 'generating', ретраи не срабатывают.
# С таймаутом зависший вызов прерывается → засчитывается как неудачная попытка → ретрай.
GEMINI_TIMEOUT_MS = int(os.getenv("GEMINI_TIMEOUT_MS", "180000"))  # 3 мин на вызов
IMAGE_MAX_LONG_SIDE = 2000       # px, resize before sending to Gemini
UPLOAD_MAX_BYTES = 15 * 1024 * 1024

# --- Фремиум-бета: бесплатный разбор одного рисунка (projectSpec/fremium) ---
# Модель по умолчанию ТА ЖЕ, что в платном отчёте: смысл беты — увидеть, что пишет
# на реальных рисунках рабочая модель, а не её дешёвый заменитель. Переключение на
# Flash возможно из .env без правки кода, если замер стоимости заставит.
FREE_GEMINI_MODEL = os.getenv("FREE_GEMINI_MODEL", GEMINI_MODEL)
# Вывод в ~10 раз короче платного отчёта, поэтому и таймаут короче: родитель ждёт
# на экране, а 3 минуты ожидания — это потерянный человек, а не спасённый заказ.
FREE_GEMINI_TIMEOUT_MS = int(os.getenv("FREE_GEMINI_TIMEOUT_MS", "90000"))
FREE_MAX_ATTEMPTS = 3            # не 5: родитель ждёт, и каждая попытка стоит денег
FREE_REPAIR_ROUNDS = 1           # дальше — даунгрейд (снять гипотезу), а не бесконечный ремонт
FREE_DIR = DATA_DIR / "free"           # data/free/{analysis_id}/...
FREE_LAB_DIR = DATA_DIR / "free_lab"   # офлайн-прогоны scripts/free_lab.py
FREE_WORKER_LOG = DATA_DIR / "free_worker.log"
FREE_MAX_CONCURRENT = 2          # одновременных генераций (проверяется запросом в БД)
FREE_STALL_SECONDS = 300         # 'running' дольше этого = зависла, реапер вернёт в очередь
# Срок хранения ФОТО бесплатной загрузки. Текст разбора живёт дальше — в нём нет
# изображения, и он и есть цель беты. Платный заказ не страдает: при переиспользовании
# файл копируется в data/drawings/{order_id}/ и живёт своей жизнью.
FREE_IMAGE_TTL_DAYS = 90
# Суточный потолок бесплатных разборов. Включён с первого дня: прямую ссылку будут
# раздавать в родительских группах и телеграм-каналах. 0 = без потолка.
# Потолок стоит ПРОТИВ СКРЕЙПЕРА, а не против расходов: замер дал 1.35 ₽ за разбор,
# то есть 200/сутки — это ~270 ₽ в худший день, а заблокированный живой родитель
# стоит 1490 ₽. Экономить здесь дороже, чем не экономить.
