      ...
      fetch("/free/upload/" + token, { method: "POST", body: fd })
        ...
        .then(function (res) {
          if (!res.ok) {
            ...
            err.textContent = ({
              cap: (res.j.saved ? "На сегодня разборы закончились. Мы отправили на почту ссылку — вернитесь по ней завтра, и рисунок разберём."
                                : "На сегодня разборы закончились. Укажите почту — пришлём ссылку, по которой завтра можно вернуться."),
