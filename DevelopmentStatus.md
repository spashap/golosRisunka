...
## UseCase — прод 500 после деплоя карточек примеров (23.06)
Симптом: после `./deploy.sh` (без install_samples) лендинг/`/primer`/sitemap отдают 500.
Причина: `app/samples.py` при сборке генерирует webp-миниатюры в `static/img/samples/`. `git pull`
кладёт закоммиченные миниатюры от имени root, а веб-процесс (www-data) при первом запросе
ПЫТАЕТСЯ их перезаписать → PermissionError → 500 на всех страницах, зовущих get_samples().
Фиксы: (1) код — `_thumb_file` отдаёт уже существующую миниатюру КАК ЕСТЬ (не перезаписывает),
а `_load` оборачивает каждый образец в try/except (сломанный образец пропускается, не роняет
страницу) — V1.035. (2) немедленный анблок на сервере:
`chown -R www-data:www-data /var/www/golosrisunka/static/img && systemctl restart golosrisunka-web`.
Правило: любые генерируемые в static/ файлы, что коммитятся, после code-only деплоя могут быть
root-owned — либо не перезаписывать существующие, либо chown в deploy.


## Состояние на 23.06.2026 — РАЗВОРОТ К ФИЛОСОФИИ 2.3 «ПОРТРЕТ РЕБЁНКА» (сессия, V1.037)
ПОЛНЫЙ пивот ядра продукта: с «только навыки рисования» на «портрет ребёнка как личности через рисунок».
Триггер: реальная мама ЦА (Юлия) — слоган обещал голос ребёнка, а отчёт давал арт-измерение. После пивота
она одобрила: «100% про Лизу!». Заказчик одобрил и отчёт, и лендинг.

ЧТО СДЕЛАНО (по версиям):
- V1.028 Задача 1 — пайплайн отчёта 4.0: рамка «ребёнок как личность»; зона-3 в 4-условной оправе
  (атрибуция+гипотеза+деталь+возврат к ребёнку); блок about_child; 7 новых направлений (world_and_themes/
  character_in_line_color/mood_and_expression/story_and_characters ведут + creativity/technique_and_materials/
  fine_motor поддержка); understanding_/art_recommendations; specialists; механика «тумана»; ЛИНТЕР переписан
  с «бан слов» на «проверку оправы» (HARD везде + frame только интерпретация; repair=добавь-оправу-не-удаляй);
  схема +about_child/специалисты/раздельные реки; шаблон отчёта + админ-тексты конца (config/report_texts.json
  + /admin/report-texts + get_report_texts).
- V1.029 Задача 1.1 тюнинг (по Лизе): разброс оценок (не стена 9), атрибуция ≤2x/автор + родовая безымянная,
  специалисты неарт-вариант, меньше суперлативов, about_child≠mood; линтер B1 артефакт-исключение для тяжёлых.
  + Unisender custom_backend_id=31525 (Return-Path=click.golosrisunka.ru, фикс DKIM/спам; нужен env на сервере).
- V1.030 Задача 2 — лендинг: hero сохранён; новые «Больше чем рисунок» + «Например, такие ситуации»
  (иллюстративные, НЕ реальные клиенты); «Без мифов»→«Бережно и по методикам, не гадание»; FAQ/мета/JSON-LD
  под новый голос (SEO-ключи сохранены); 2 «навыковых» отзыва убраны; дисклеймеры = позитивная идентичность.
- V1.031 карточка примера → прямой /r/<token> (минуя /primer-обёртку).
- V1.032 фьюент-правка текста лендинга (ревью ChatGPT, применено 37 блоков, 1 придержан; через
  projectSpec/pipe-change/landing-text-review*.json).
- V1.033 точечный реворд («один рисунок помогает увидеть момент, серия — более полную картину»).
- V1.034 редизайн карточки примера: крупный полароид рисунка + цитата about_child + мелкие оценки; сводный
  по 2 рисункам — два полароида веером; адаптив. V1.036 — фикс выравнивания мульти-полароида (фикс-высота
  зоны → имя/оценки выровнены), крупнее+плотнее+cover.
- V1.035 фикс прод-500: `_thumb_file` не перезаписывает существующую миниатюру (git-pull кладёт от root,
  www-data не мог перезаписать → 500); `_load` пропускает сломанный образец.
- (no bump) Задача 1.2 — development_directions → «куда растить ребёнка ПО ЖИЗНИ» (3 слоя: черта→развитие в
  жизни→сфера+профессии «как пример»); заголовок «Куда развивать сильные стороны ребёнка», усилена оправа.
- (no bump) Gemini per-request таймаут GEMINI_TIMEOUT_MS=180с (base_url прокси сохранён) + пошаговый трейс
  в worker.log + scripts/gemini_ping.py. Причина: зависший вызов блокировал воркера навсегда (заказ висел в
  'generating', ретраи не срабатывали — нет таймаута). Подтверждено транзиентным зависанием Gemini-прокси.
- (no bump) Линтер-тюн по ложным срабатываниям из новой лексики: hedge += могут/можем/предположить;
  WORK_REF += «геро»; frame-check ТОЛЬКО на интерпретационных полях (intro/about_child/conclusion/observation/
  specialists.reason), НЕ на context_summary/рекомендациях/направлениях (там «настроение героя» легитимно).
- Примеры отчётов перегенерированы пайплайном 4.0 (4 шт), 0 нарушений после тюна; на прод тарболом.
- V1.037 комбобокс формы заказа (тема/материалы): нативный datalist на мобильных не давал ВЫБРАТЬ вариант →
  свой комбобокс в order.js (выбор по mousedown + клавиатура, свободный ввод сохранён), .combo-list в
  components.css. Покрыты все 6 инстансов (2 поля × 3 блока), нативный list= убран везде.
- Хендофф для англо-версии: projectSpec/pipe-change/HANDOFF-english-philosophy-2.3.md (концепт+оправа+линтер+
  лендинг+ops + КАВЕАТ: англо-рынок строже к псих-интерпретации, зону-3 приглушить).

ЧЕГО ЖДЁМ / PENDING:
1. Задеплоить хвост: V1.037 (комбобокс) + линтер-тюн + Gemini-таймаут/трейс + Задача 1.2 → на сервере
   `./deploy.sh`; образцы → scp golos_samples.tar.gz + `install_samples.sh`.
2. Добавить `UNISENDER_GO_CUSTOM_BACKEND_ID=31525` в СЕРВЕРНЫЙ .env (фикс спама), затем проверить заголовки
   письма (Return-Path/DKIM=golosrisunka.ru).
3. СБП на мобильном: embedded-виджет ЮKassa даёт только QR, без перехода в банк-приложение. Текст вопроса
   посреднику составлен (ждём ответ ЮKassa — возможно нужен redirect-флоу вместо embedded). Не баг кода.
4. 3 правки feature-копий в products.json — заказчик через /admin/settings (чтобы не было конфликта деплоя).
...
## 02.07.2026 — Админка «Цены» + фикс прод-500 записи конфигов из веба (V1.047–V1.048, В ПРОДЕ)
**Задача заказчика:** цены управляются из админки; на сайте две цены — до скидки (зачёркнутая)
и после скидки; ЮKassa получает цену после скидки минус промокод.

**Сделано (V1.047):**
- Новый раздел сайдбара «Цены» → `/admin/prices` (между «Промокоды» и «Настройки сайта»):
  у каждого продукта поля «цена до скидки, ₽» и «цена к оплате, ₽» + живой пересчёт «Скидка на сайте: −N%»
  (инлайн-JS). Валидация: целые числа; «до» строго выше «к оплате»; пустая «до» = зачёркнутая цена
  не показывается (ключ `old_price_rub` удаляется). При ошибке — redirect с сообщением, файл не трогается.
- Ценовые поля УБРАНЫ из «Настроек сайта» (одна страница-источник; настройки больше не трогают цены —
  проверено, что их сохранение не затирает ценовые ключи).
- `config/products.json`: добавлены `old_price_rub` 4900 (snapshot) / 6900 (development) по модели продукта.
  Рендер обеих цен на лендинге/форме заказа уже существовал (`p-old`/`p-price`, гард `{% if p.old_price_rub %}`).
- Платёжную цепочку НЕ трогали: `orders.py` создаёт заказ по `price_rub`×100, промокод режет процентом,
  ЮKassa получает `order.price_kopecks` с проверкой суммы на webhook — как и требовалось.
- dist/ пересобран (лендинг теперь рендерит зачёркнутую цену).

**Инцидент после деплоя V1.047:** «Сохранить цены» в проде → 500. По `data/web.log` на сервере:
`PermissionError: [Errno 13] /var/www/golosrisunka/config/products.json` — репо на проде это root-owned
git clone, веб-процесс www-data владеет только `data/` и `static/img/`. Латентно сломаны были и
«Настройки сайта», и «Тексты отчёта» (с их создания — просто никто не жал Save в проде). Вторая ловушка:
даже с chown правка git-tracked файла на сервере ломала бы следующий `git pull` в deploy.sh.

**Фикс (V1.048, UseCase #28) — оверлей в data/:**
- Админ-правки пишутся АТОМАРНО (tmp + rename, `_atomic_write_json`) в `data/products.json` /
  `data/report_texts.json` (www-data-owned, в .gitignore → git pull не конфликтует).
- `settings.get_products()` / `get_report_texts()`: приоритет data/-оверлея, фолбэк на git-дефолт
  `config/*.json`; mtime-кэш теперь ключуется (путь, mtime) — правки видны без рестарта, web и worker
  читают одинаково. До первого сохранения прод живёт на config-дефолтах — ничего не теряется,
  первое сохранение копирует всё в data/.
- E2E-тест (32 проверки): рендер обеих цен на лендинге/форме, roundtrip сохранения, валидации,
  config-дефолт не тронут, «Настройки» не затирают цены, «Тексты отчёта» в data/.
**Правило (в CLAUDE.md): всё, что пишет админка — ТОЛЬКО в `data/`; config/ = read-only дефолты.**
Задеплоено заказчиком, сохранение цен в проде проверено — работает.


## 13.08.2026 — Аналитика фремиума: блок на дашборде + раздел «Фремиум» (локально, НЕ задеплоено)
**Задача заказчика:** на дашборде — сколько людей прошло фремиум с делением «оставили только email» /
«запросили бесплатный разбор»; отдельная страница — насколько глубоко доходят, сколько фремиума
привело к покупке, что выбирают в анкете.

**Сделано:**
- `app/admin_free_analytics.py` (новый) — весь счёт; маршрут в admin.py тонкий.
  Две единицы измерения намеренно НЕ смешаны в одной колонке: шаги мастера строк в БД не оставляют
  и считаются по уникальным ПОСЕТИТЕЛЯМ (клики приходят маяком `/t/e` как `click:<goal>`),
  а путь до разбора считается по АНКЕТАМ (строки `free_analyses`). Поэтому воронки две.
- Дашборд (`/admin/analytics`): блок «Фремиум» — прошли анкету / только email (рисунка нет) /
  запросили разбор / разбор готов / ушли без почты и без рисунка + ссылка на раздел.
  «Только email» = `status='answers' AND email IS NOT NULL` (ветка «нет рисунка под рукой»);
  «запросили разбор» = `status <> 'answers'` (рисунок реально загружен).
- Новый раздел сайдбара «Фремиум» → `/admin/free-analytics` (рядом с «Бета»; «Бета» — про КАЧЕСТВО
  текстов и библиотеку трактовок, «Фремиум» — про ПОВЕДЕНИЕ). Содержимое: KPI; воронка по странице
  (8 шагов от `free_view` до клика «заказать отчёт»); воронка по анкетам (анкета → рисунок → разбор →
  заказ → оплата); что делают с готовым разбором (оценки, «ещё рисунок», лимит); кто остановился на
  анкете (с почтой / молча); отказы и флаги; покупки после фремиума; что выбирают в анкете.
- «Что выбирают в анкете» — 6 таблиц (тревога, срок, возрастная полоса, обращение, свободный текст,
  вариант просьбы). У каждого варианта не только «сколько выбрали», но и куда эти люди дошли
  (загрузили / получили разбор / заказали / оплатили): часто выбираемый вариант, после которого не
  несут рисунок, — это проблема текста, а не спроса. Варианты с нулём остаются видимыми (ноль — ответ).
- **Атрибуция «фремиум → покупка»**: добавлена колонка `orders.free_token` (миграция в `_migrate`)
  и запись токена в `routes.order_submit` — точная связь для перехода из разбора. Косвенные склейки
  (тот же email / тот же посетитель, заказ не раньше анкеты) считаются отдельными строками и подписаны
  как догадка. Один заказ достаётся ОДНОЙ анкете — последней до заказа, иначе два разбора одного
  человека дали бы две «покупки».
- Период страницы = дата АНКЕТЫ (когорта); покупки когорты считаются без ограничения по дате —
  вопрос «купил ли вообще». Написано в подсказке на странице.
- Проверено: тест на копии боевой БД с досеянными сценариями (done/queued/rejected, голос,
  три типа атрибуции, боты) — обе страницы 200, цифры сходятся; боты отсечены (`NOT_BOT`).
- Новых CSS-классов НЕТ намеренно (админ-шаблоны отдают css без `?v=`, 30-дневный кэш nginx —
  новый класс не доехал бы до заказчика). Всё собрано из существующих компонентов админки.

**Дополнение (тот же день):** родители из фремиума видны в списках админки.
- «Клиенты»: вкладки Все / Покупатели / Из фремиума (со счётчиками) + колонка «Разборов»
  и метки «только фремиум» (лид) / «фремиум → покупка». Строки в `customers` фремиум создавал
  и раньше (`free._link_customer`), но выглядели пустыми: детей он намеренно не заводит,
  поэтому имя ребёнка для таких строк берём из самого разбора (`free_analyses.child_name`).
- «Заказы»: вкладка **«Бесплатные разборы»** (`?tab=free`) — разборы, где родитель оставил почту:
  email, ребёнок, тревога, «что оставил» (только почта / почта и рисунок), статус, ссылка на разбор
  и колонка «Купил» (та же атрибуция, что на странице «Фремиум», через `fa.purchases_index`).
  В одну таблицу с платными НЕ сводим: разные сущности и разные единицы измерения.
- Макрос вкладок `templates/admin/_tabs.html` (те же пилюли, что фильтр периода — новых CSS нет).
  ⚠️ Имя переменной контекста — `tabs_items`: `tabs` затирается импортом самого макроса.


## 14.08.2026 — ПЕРЕСБОРКА АНАЛИТИКИ, этапы 0–1 (локально, НЕ задеплоено)
**Задача заказчика:** «аналитика ничего не стоит — нет настоящей воронки, нет глубины
прокрутки, нечем измерить, что улучшать». Разбор показал, что это не косметика.

**Диагноз (проверено по коду, не на глаз):**
1. Не было ВИЗИТА. `events` — плоский лог, а админка считала визит как
   `COUNT(DISTINCT visitor_id)` за период: пришедший пять раз за месяц = «1».
   Ни длительности, ни страницы входа/выхода, ни времени до покупки.
2. Воронка не была воронкой: девять НЕЗАВИСИМЫХ множеств делились друг на друга,
   шаги не вложены, фремиум-двери в ней не было вообще, а `order_paid`/`report_delivered`
   приходят от воркера без visitor_id и попадали туда COALESCE-трюком.
3. У событий не было АДРЕСА страницы: маяк слал только имя цели, поэтому «на какой
   странице произошло» приходилось угадывать по префиксу имени.
4. Блог (14 SEO-статей) не оставлял вообще ничего: ни просмотра индекса, ни статьи.
5. Глубины прокрутки не существовало: `engaged` — двоичный (первое действие ИЛИ 15 c).
6. UTM только первого касания и на год: клик Директа, который реально принёс покупку,
   приписывался органике полугодовой давности; `yclid` не сохранялся.
7. Отказы не измерялись: обрывы загрузки во фремиуме, клиентская валидация заказа,
   блокировка отправки из-за опечатки в email, повторные попытки оплаты.
8. **Полсотни `data-ym-goal` слали `reachGoal` в пустоту**: Метрика считает конверсию
   только по цели, СОЗДАННОЙ в счётчике. Значит, в отчётах ноль и Директу не на чем учиться.

**Этап 0 — модель данных (сделано):**
- `web_visits` + `events.visit_id` + `events.path` + `orders.visit_id`. Визит — cookie
  `gr_s` на 30 минут, продлевается на каждом запросе; отсутствие куки И ЕСТЬ новый визит
  (никаких сравнений времени на сервере). Строка визита пишется в `after_request`, где уже
  известен тип ответа: маяки/поллинг/статика страницами не считаются.
- Метки визита: канал (`ads/organic/social/referral/internal/direct` — `classify_channel`),
  UTM ЭТОГО визита (последнее касание) отдельно от первого касания, `yclid`.
- Ширина экрана и тач приходят ОТ КЛИЕНТА (`sw`/`t` в маяке): верстка ломается по ширине,
  а user-agent не знает ни про узкое окно, ни про планшет в режиме десктопа.
- `mark_visit()` обновляет engaged / max_scroll (через MAX — маяки приходят не по порядку) /
  screen_w / customer_id.
- Мелкие находки по дороге: `order.js` грузился БЕЗ `?v=`; CSS админки — тоже без `?v=`
  (поэтому в неё и не писали новых классов); `.hint` стоял в каждом шаблоне админки, но
  правило было только у `.field .hint` — вне поля подсказки рисовались обычным текстом;
  `worker.py` не писал heartbeat, хотя админка его показывала (порог для платного воркера
  поднят до 600 c — он молчит всё время генерации).

**Этап 1 — что теперь измеряется (сделано):**
- **`static/js/track.js`** — своя аналитика ОТДЕЛЬНО от счётчика Метрики. Раньше маяк и
  цели сидели внутри `{% if metrika_id %}`, и без чужого ID собственная аналитика молча
  выключалась целиком. Теперь `_metrika.html` — только счётчик + `window.GR_YM_ID`.
  Лендинг НЕ наследует `_base.html`, поэтому скрипт подключён и там отдельно.
- **Глубина прокрутки** 25/50/75/100 (от прокручиваемого расстояния; на непрокручиваемой
  странице не шлём ничего) и **видимость блоков**: 13 секций лендинга + тело статьи
  (`data-track-section`, IntersectionObserver, полсекунды на экране и половина блока —
  инерционный свайп на мобильном не должен считаться «видел цены»).
- **`config/goals.py`** — реестр всех целей с подписями, группами и флагом «в Метрику».
  В Метрику идёт осмысленный набор (~50), длинный хвост остаётся только в своей базе.
  `scripts/metrika_goals_sync.py` создаёт недостающие цели через Management API
  (идемпотентно, сверка по ИДЕНТИФИКАТОРУ, `--dry-run`, ничего не удаляет).
  `app/metrika.py` — клиент на stdlib `urllib` по образцу `yookassa.py`.
- **События сбоев**: все отказы загрузки во фремиуме (`free_upload_failed` с причиной),
  клиентская валидация и слишком большое фото в обеих формах, блокировка отправки из-за
  опечатки в email, возврат к брошенному черновику, обрыв сети при загрузке, ожидание
  дольше 90 c и исчерпание ожидания, повторная попытка оплаты, 404/500, просмотры блога,
  юридических страниц, страницы «заказ принят» и КУПЛЕННОГО отчёта, неудачные входы.
  Диагностические маяки оплаты теперь несут `order_id`.
- **`app/admin_funnels.py` — воронки по ВИЗИТАМ, две двери.** Ключевое решение: шаги
  двух видов. ВОРОТА (открыл форму, дошёл до оплаты) достраиваются по самым дальним
  достигнутым — недолетевший маяк не рвёт воронку, вложенность гарантирована.
  НАБЛЮДЕНИЯ (доскроллил до цен) считаются как есть: человек мог прийти по прямой ссылке
  сразу на форму, и записать ему «видел цены» ради красиво убывающей таблицы значило бы
  соврать. Переход между блоками может быть >100% — это находка, а не ошибка.
  «Оплатил» берётся из ЗАКАЗА (`orders.visit_id` + `paid_at`), а не из события: оплату
  подтверждает вебхук, в котором браузера нет. Мобильный — по ширине экрана, если она
  есть. Таблица каналов (последнее касание) заменила прикидку по first-touch UTM.
- **`scripts/analytics_selftest.py`** — самопроверка на КОПИИ боевой базы: 30 проверок
  (каналы, визит, маяки, отказы, полнота реестра целей, вложенность ворот, отсутствие
  достройки наблюдений, исключение ботов). Все зелёные.

**⏳ Дальше (этапы 2–3, нужен OAuth-токен Яндекса от заказчика):** атрибуция кампаний с
РАСХОДОМ Директа (Reporting API — расход наша база знать не может), раздел «Кампании»,
суточная синхронизация без нового systemd-юнита (по образцу суточного прохода в
`free_worker`), Вебмастер (запросы/показы/позиции) → раздел SEO.

**Уточнение по доступам (14.08):** Директом управляет ПОДРЯДЧИК, доступа к рекламному
кабинету у заказчика нет; Вебмастер — есть. Следствия для этапов 2–3:
- расход/CPA по кампаниям из Метрики придут ТОЛЬКО если подрядчик свяжет рекламный
  аккаунт со счётчиком 109824945 — проверяется `scripts/yandex_token_check.py`;
- атрибуция «реклама → заказ → деньги» считается НА НАШЕЙ СТОРОНЕ по `yclid`
  (Директ дописывает его к ссылке) и не зависит от отчётности подрядчика — это
  единственная независимая проверка его работы. Без UTM/связки `yclid` даёт «это Директ»,
  но не имя кампании;
- инструкция по токену и текст просьбы подрядчику: `projectSpec/yandex-api-token-howto.md`;
- добавлены `app/webmaster.py` (v4: user_id, hosts, popular_queries, summary) и
  `scripts/yandex_token_check.py` (ничего не меняет: проверяет токен, права на цели,
  наличие связки с Директом, печатает host_id для .env).

**Раздел «Задачи» (`/admin/todo`, первым в сайдбаре):** то, что делается РУКАМИ и чего
нет в коде (цели в Метрике, доступы, просьбы подрядчику) — из переписки такие задачи
пропадают. Таблица `admin_tasks`, свёрнутый список (`<details>`, без JS): свёрнуто —
только название и статус, развёрнуто — полная инструкция. Первая задача засеяна
автоматически: «Завести 14 целей в Метрике» с текстом, который СОБИРАЕТСЯ ИЗ РЕЕСТРА
`config/goals.py` (руками список не дублируем — разъехавшись, он отправил бы заводить
несуществующие цели). Засеянные задачи не удаляются, только закрываются: удалённая
вернулась бы при следующем открытии страницы. **Решение по объёму: API Вебмастера и
Директа НЕ подключаем** — Директ ведёт подрядчик, доступа нет; данные Вебмастера
целиком видны в его же интерфейсе, и API дал бы только удобство. Токен не нужен,
`app/webmaster.py` и `scripts/yandex_token_check.py` остаются в репозитории мёртвыми.


## 14.08.2026 — ПОСАДОЧНАЯ `/free-check` под холодный платный трафик (локально, НЕ задеплоено)
**Задача заказчика:** отдельная страница под рекламу Meta/Instagram, единственная цель —
перевести в бесплатный мастер `/free/`. Ни цен, ни платного продукта, ни методологии.
Тексты — авторские, использованы дословно.

**Решения заказчика по ходу:** пункт «Бесплатная проверка» идёт в ГЛОБАЛЬНУЮ шапку (поправка
к правилу «бесплатно — не имя продукта», записана в CLAUDE.md); страница ИНДЕКСИРУЕТСЯ;
карточки тревог передают выбор в мастер; кнопка шапки на этой странице подменяется на
бесплатную; добавлена 8-я карточка «перестал рисовать» (в мастере такой вариант есть,
в присланном списке его не было); герой — то же фото, что на главной.

**Две ловушки, найденные ДО реализации (иначе сломалось бы тихо):**
1. `.reveal` работает ТОЛЬКО на лендинге: класс объявлен `opacity:0`, а наблюдатель,
   добавляющий `.is-in`, живёт внутри инлайн-скрипта `landing.html`. Тот же блок на любой
   другой странице остался бы невидимым навсегда. → страница сделана без `.reveal`
   (заодно выполняет требование «без тяжёлых анимаций»), скрипт главной не тронут.
2. Цель Метрики `page_free` срабатывала по подстроке `/free` — то есть считала бы и
   `/free-check` входом в мастер. → условие ужато до `/free/`, заведена отдельная
   `page_free_check`.

**Сделано:**
- `templates/free_check.html` (наследует `_base.html`) — hero, 8 карточек-тревог, 3 шага,
  блок «нет рисунка под рукой», блок любопытства, блок доверия, финальный призыв.
  Только существующие компоненты: `.hero`, `.case-list`, `.steps`, `.worry-band`, `.free-band`.
  Новых классов ровно три и все — недостающие части компонентов: `.case--link` (карточка
  стала кликабельной и обязана выглядеть нажимаемой), `.hero__note` (строка под кнопкой),
  `.cta-row` (одиночная кнопка по центру секции — иначе её пришлось бы центрировать
  инлайн-стилем, что запрещено бренд-буком).
- Все призывы ведут на `/free/` СО СЛЕШЕМ: ссылки вида `/free` дают 308-редирект, и каждый
  платный клик тратил бы лишний переход.
- `_header.html` параметризован: кнопка читает `header_cta` с ДЕФОЛТАМИ, равными прежним
  зашитым значениям, — все остальные страницы рендерятся как раньше (проверено тестом).
  Пункт меню добавлен глобально. ⚠️ `.site-nav` скрыт ≤880px, поэтому на мобильных пункт не
  виден по существующему дизайну шапки; на самой посадочной роль играет подменённая кнопка.
  Для полосы 881–1000px добавлена медиа-правка: пятый пункт перестал помещаться, а `body`
  с `overflow-x: clip` обрезал бы кнопку справа вместо скролла.
- `static/js/free.js`: `?concern=<key>` принимается, только если такой радио-вариант реально
  есть в мастере (чужая ссылка не положит в анкету неизвестный серверу ключ); выбор
  проставляется, и на шаге «имя и возраст» мастер пропускает экран выбора тревоги
  (`pickConcern` → шаг «как давно»). Одноразово: вернувшись назад, человек может выбрать
  другое. Цель `free_concern_preset` — чтобы видеть, как часто работает deep-link.
- Аналитика: `free_check_view` + 6 целей по позициям призыва + 8 целей по карточкам +
  7 секций `fc_*`, всё в `config/goals.py`. **Третья дверь в воронках**
  (`admin_funnels.COLD_STEPS`, вход `free_check_view`): посадочная → призыв → мастер →
  анкета → рисунок → заказ → оплата. Ровно это и позволяет сравнить «реклама → посадочная»
  с «реклама → главная».
- SEO: в sitemap (0.9), canonical/OG считаются автоматически из `_base.html`; robots не
  трогали (`Allow: /` + явные Disallow). `export_static.py` — новая страница в зеркале Vercel.
- `scripts/analytics_selftest.py` дополнен проверкой шапки: подмена кнопки не должна
  протечь на другие страницы. Всё зелёное.

**⏳ Проверить заказчику на телефоне:** переход по карточке (тревога должна оказаться уже
выбранной, экран выбора — пропущен) и вид шапки на ноутбуке шириной ~900px.

**Правка того же дня (по итогам проверки заказчиком):** deep-link `?concern=` УБРАН целиком.
На `/free/?concern=black#name` кнопка «Дальше» не срабатывала; корневую причину не искали —
заказчик решил, что двойной выбор (карточка на посадочной + тот же вопрос в мастере) сам по
себе лишнее усложнение и два списка пришлось бы вечно держать синхронными. Сделано:
- `static/js/free.js` возвращён к состоянию до V1.069 — файл БАЙТ-В-БАЙТ равен версии,
  которая работает в проде с 13.08 (проверено `git diff` против предыдущего коммита);
- карточки на `/free-check` стали обычным ТЕКСТОМ (`div`, не ссылки): человек узнаёт свою
  ситуацию и идёт по ОДНОЙ кнопке. Выбор тревоги делается один раз — в самом мастере;
- копия под карточками переписана: «Что-то из этого про вас? Ответьте на несколько коротких
  вопросов — и получите бесплатную трактовку рисунка», кнопка «Получить бесплатную трактовку»;
- удалены ставшие мёртвыми 8 целей `freecheck_card_*`, цель `free_concern_preset` и CSS
  `.case--link`. В теле страницы остался ровно один адрес назначения — `/free/#name`.
**Правило на будущее:** список тревог существует в ОДНОМ месте — `config/free_texts.CONCERNS`.
Дублировать его как интерактивный выбор где-то ещё нельзя: два списка расходятся молча.

**Итог дня 14.08.2026 (V1.065 → V1.070, всё в проде).** Три блока работы: аналитика фремиума
(дашборд + раздел «Фремиум» + фремиум-родители в «Клиентах»/«Заказах»), пересборка измерений
(визиты, воронки по визитам, глубина прокрутки, события отказов, реестр целей, раздел «Задачи»)
и посадочная `/free-check` под холодный трафик с третьей дверью в воронках.
**Что нужно от заказчика:** (1) завести 14 целей в Метрике руками — задача стоит в `/admin/todo`;
(2) проверить `/free-check` на телефоне и шапку на ~900px; (3) отдать подрядчику просьбу про UTM
и связку Директа со счётчиком (текст в `projectSpec/yandex-api-token-howto.md`);
(4) для Meta/Instagram размечать объявления UTM — у Meta нет аналога yclid.
**Осознанно НЕ делаем:** API Вебмастера/Директа (нет доступа к рекламному кабинету, данные
Вебмастера видны в его же интерфейсе). Токен не нужен; `app/metrika.py`, `app/webmaster.py`,
`scripts/yandex_token_check.py` лежат мёртвыми до тех пор, пока заказчик не заберёт Директ себе.

## 02.09.2026 — ОТЗЫВЫ: звёзды 1–5 + текст на разборе и отчёте, колонки в админке, раздел «Отзывы» (V1.072)

**Задача заказчика (шаг 1 из серии про обратную связь):** (A) у каждого, кто получил бесплатный
разбор, — оценка звёздами 1..5 и необязательный текст; (B) то же для платного отчёта по
реальному рисунку; (C) в «Клиентах» — колонка с оставленным отзывом и, для фремиум-клиентов,
ссылка на то, что мы выдали (как для платных отчётов). Задеплоить, поднять версию.

**Что сделано.**
- **Одна таблица `feedback`** (`kind` free/order, `ref_id`, `stars`, `text`, `customer_id`,
  `visitor_id`, `created_at`, `updated_at`; уникальный индекс по (kind, ref_id)). Одна оценка на
  продукт, повтор — UPDATE. Отдельно от `free_interpretations.parent_vote`: там «про него /
  не про него» по конкретной ФРАЗЕ, здесь «помогло ли» про результат целиком.
- **`app/feedback.py`** — блюпринт `/feedback/free/<token>` и `/feedback/order/<public_token>`
  (POST, доступ по токену продукта — как и у самих страниц), помощники `get_one`/`index_for`/
  `stars_str`, `widget_html` + `inject_into_report`. Событие `feedback_left` {kind, stars,
  with_text} в свою аналитику.
- **Один виджет** `templates/_feedback_widget.html` + `static/js/feedback.js` + компонент
  `.rate` в `components.css` (токены; звёзды через DOM 5→1 + `row-reverse`, чтобы `:checked ~`
  заливал «до выбранной»; подписи шкалы: 1 «Не помогло» … 5 «Очень точно, узнали ребёнка»).
  На `/free/r/<token>` — между оценками фраз и продающим блоком (ДО оффера: мнение свежее).
  На `/r/<token>` платного отчёта — **дописывается при отдаче** перед `</body>`
  (отчёт лежит готовым файлом; в PDF формы нет; образцы лендинга виджет НЕ получают).
- **Кабинет**: у готовых отчётов и разборов строка «Ваша оценка ★★★★☆ · изменить» или
  ссылка «Оценить …» (якорь `#feedback`). Письма `free_ready`/`report_ready` — по одной
  фразе-приглашению оценить внизу страницы.
- **Админка**: «Клиенты» — колонки «Получил» (ссылки на каждый готовый отчёт `/r/…` и разбор
  `/free/r/…` со звёздами рядом) и «Отзыв» (последний + «+N», текст); «Заказы» — колонка
  «Оценка» в обеих вкладках; **новый раздел «Отзывы»** (`/admin/feedback`, фильтр периода):
  по каждому продукту отдельно средний балл, отклик (оценок из выданных), распределение 1..5,
  ниже — лента отзывов со ссылками на продукт. Цели `free_rate_submit`/`report_rate_submit`/
  `cabinet_rate_report`/`cabinet_rate_free` заведены в `config/goals.py` (в Метрику НЕ идут —
  это не конверсии). `scripts/analytics_selftest.py` — ALL OK.
- **Проверка**: смоук на копии БД (26 проверок: upsert, обрезка текста 1500, 404 на чужой
  токен, вставка в отчёт без поломки `</body>`, образцы без виджета, все страницы админки 200,
  кабинет со звёздами) + живая проверка виджета в браузере на localhost.
- **`deploy.sh`: `SKIP_PULL=1`** — пропуск `git pull` на время, пока аккаунт GitHub скрыт
  (см. ниже).

**GitHub-инцидент (с 28.08.2026, актуально на 02.09).** Аккаунт `spashap` помечен и скрыт:
анонимный HTTPS-клон прода (`/var/www/golosrisunka`, remote `https://github.com/...`)
отвечает «could not read Username», deploy-ключи тоже не читают (проверено на CosmyDay).
Push/pull с машины заказчика по SSH работает. Свод правил — `C:\projects\CosmyDay\CLAUDE.md`
§1e. **Временный деплой без дрейфа истории**: `git push fornex main:refs/heads/claude-deploy`
(remote `ssh://root@31.172.67.220/var/www/golosrisunka`) → на сервере `git merge --ff-only
claude-deploy` → `SKIP_PULL=1 ./deploy.sh`. После восстановления аккаунта обычный `./deploy.sh`
снова делает pull сам.


## 02.09.2026 — ШАГ 2 РЕВИЗИИ: критические исправления (V1.074)

**Основание:** полная ревизия сайта и админки (отчёт-артефакт, память `audit-2026-09-02`):
четыре прохода — краулинг прода, read-only аудит боевой БД, ревью кода админки, CRO-обход.
Заказчик: «start step 2», юридические тексты — взять с shepotzvezd.ru (та же ИП-партнёр).

**C1 · Утечка email по номеру заказа — закрыто.** `/order/success/<id>`, `/pay/<id>`,
`/pay/yookassa/status/<id>`, `/pay/yookassa/create/<id>` отдавались любому по порядковому
номеру. Теперь у заказа `orders.access_token` (миграция), он живёт в куке `gr_o<id>` браузера,
который оформил заказ (ставится при создании и при возврате с банка); доступ также у вошедшего
владельца. Без доступа: неоплаченный → 404, оплаченный → редирект на `/login` (пришёл с
другого устройства). `/pay/yookassa/return` доступа не требует (кука могла остаться в другом
браузере), но email не показывает.

**C2 · Юридика — настоящие документы.** `templates/legal_{offer,privacy,consent,contacts}.html`
поверх макета `legal.html`: оферта (услуга = информационные материалы по рисункам, НЕ
диагностика/психология; возврат 7 дней как обещано на лендинге; бесплатный разбор — §6.4),
политика 152-ФЗ (что собираем и зачем, фото 90 дней, IP не храним, подрядчики, Метрика),
согласие, контакты с реквизитами. Реквизиты — `settings.LEGAL` (ИП Семеш А. Я., ОГРНИП/ИНН/адрес
как на shepotzvezd.ru), почта поддержки — `settings.SUPPORT_EMAIL` (по умолчанию =
`MAIL_FROM_EMAIL`, т.е. sales@; если заведут support@ — переменная `SUPPORT_EMAIL` в .env).
`/consent` — noindex, не в sitemap. Подвал `_base.html` получил ссылки Политика · Оферта ·
Контакты (раньше у всех страниц, кроме лендинга, подвал был без ссылок, а чекаут отправлял
«в подвал»). Под кнопкой формы заказа — строка согласия с офертой.

**C3 · Сессия после оплаты.** Ветки «уже оплачен» в status/return не ставили сессионную куку,
если вебхук опередил браузер → «вход выполнен» на странице успеха было ложью. Теперь
`_with_session()` зовёт идемпотентный `mark_paid()` и ставит куку.

**C4 · Второй рисунок из фремиума.** `order.js blockMissing()` считал переиспользованный
рисунок «не выбранным» (нет input[type=file]) — добавить рисунок 2/3 было нельзя.

**C5 · Сканеры больше не «люди».** `track.after_request`: новый визит начинается ТОЛЬКО со
страницы (404/маяк/robots без куки строку не создают и куку не ставят); пустой UA = bot;
404 по путям сканеров (`/.env`, `/wp-*`, `*.php`…, `track.is_scanner_path`) не пишутся в events.
Воронки (`admin_funnels`) считают только визиты, где браузер прислал ширину экрана
(`REAL_VISIT`). До этого 2 660 «человеческих» визитов при 247 настоящих.

**C6 · Тестовые данные вне KPI.** Колонка `is_test` на orders/customers/free_analyses;
миграция ставит её по `settings.TEST_EMAILS` (spashap/semesh21/jsemesh/astrometricapro),
купону YULATEST (выключен) и владельческим разборам («oleg», «тестина»). Кука владельца
`gr_ignore` (`/admin/ignore-me`, ссылка в сайдбаре «Не считать этот браузер»): визиты и
события идут как device='owner', новые заказы/разборы — is_test. KPI, источники, воронки,
счётчики фремиума и атрибуция фремиум→покупка исключают is_test; списки помечают «тест».

**C7 · Безопасность.** Все куки — `secure` на https (`settings.COOKIE_SECURE`). Токен админки —
`<ts>.<hmac>` со сроком 30 дней (старая константа не истекала и не отзывалась); лимит 8 неудачных
входов за 15 минут (таблица `admin_logins`); CSRF-поле во всех POST-формах админки
(`_guard()` проверяет). nginx: `scripts/deploy/nginx_hardening.sh` — HSTS, nosniff,
X-Frame-Options, Referrer-Policy, Permissions-Policy, server_tokens off, gzip для css/js,
www → apex 301 (идемпотентно, `nginx -t` + reload).

**C8 · Обещания, которые не выполнялись.** (а) Отказ по фото: `/free/r/<token>` со статусом
rejected показывает блок загрузки на том же токене (`free_reupload.js`), письмо ведёт туда же —
«вопросы заново проходить не придётся» стало правдой. Раньше страница отказа вообще не
достигалась (статус ≠ done уводил на экран ожидания). (б) Дневной лимит: при валидной почте
сохраняем её и шлём письмо «сохранили место» со ссылкой; текст кнопки честный.
(в) Черновик формы заказа не стирается на submit, а только на странице «заказ принят»;
«← Вернуться к форме» с оплаты ведёт на `/order?free=` при заказе из фремиума.

**Проверка:** `step2_smoke.py` (44 проверки на копии БД: миграция, юридика, правила визитов,
доступ к заказу, сессия после оплаты, троттлинг и CSRF админки, страница отказа, лимит) +
смоук отзывов + `analytics_selftest.py` — всё зелёное.
**Для заказчика:** после деплоя админка попросит войти заново (новый формат токена); в своём
браузере нажать «Не считать этот браузер» в сайдбаре; если есть ящик support@golosrisunka.ru —
добавить `SUPPORT_EMAIL` в серверный .env (сейчас в документах sales@).


## 02.09.2026 — ШАГ 3 РЕВИЗИИ: новая главная админки + определения аналитики (V1.076)

**Заказчик:** «главная страница аналитики спроектирована неправильно и не сфокусирована на
ценности для меня» → «so do it». Главная пересобрана вокруг ПЯТИ вопросов владельца.

**Дашборд (`/admin`, `app/admin_dashboard.py`, `templates/admin/dashboard.html`):**
1. *Что сломано сейчас* — полоса поверх всего: оплачено-не-доставлено >1 ч, сбои и
   «нужны другие фото» за 7 дней, зависшие/упавшие бесплатные разборы, молчащие воркеры,
   письма в outbox (в проде = не ушедшие), ошибки 500. Пусто = зелёная строка «Всё работает».
2. *Заработали ли* — выручка ЗА ВЫЧЕТОМ возвратов, оплат, средний чек, конверсия
   «визит с двери → оплата», брошенные заказы с почтой; выручка по каналам. У каждого числа
   «vs предыдущий период».
3. *Трафик* — настоящие визиты (браузер выполнил скрипт), задержались, доля телефона, топ-3
   канала; график 30 дней (столбики реклама/поиск/остальное, точки — оплаты и бесплатные
   разборы, инлайн-SVG без библиотек); входы за период.
4. *Бесплатная дверь* — недельные когорты анкета → с почтой → рисунок → разбор → заказ →
   оплата (атрибуция та же, что на «Фремиум»), медиана дней до заказа.
5. *Сколько это стоит* — таблица `ad_spend` (день × канал, вводится руками), форма ввода
   и последние записи; цена визита/лида/продажи по каналу; лид = анкета с почтой, канал лида —
   последний визит человека до анкеты.
Периоды — МОСКОВСКИЕ календарные дни (сегодня / 7 / 30 / 90), сравнение с таким же периодом
до. Всюду исключены боты, браузер владельца и is_test.

**Определения аналитики (A1–A11 из аудита):**
- A1 возвращение после паузы >30 мин со страницы сайта наследует канал/UTM/yclid прошлого
  визита того же посетителя (раньше — «internal», атрибуция рекламы терялась).
- A2 новый канал `email` (mail.google.com, e.mail.ru, mail.yandex, outlook, rambler, webmail):
  клики из наших писем считались «поиском»; `ya.ru` добавлен в поиск.
- A3 наблюдения воронки привязаны к странице (`engaged@/`, `click:scroll_50@/`): скролл статьи
  больше не «прокрутил половину лендинга».
- A4 бесплатная дверь в «Воронках» заканчивается на «отправил рисунок» (разбор и заказ —
  в другом визите); покупки из фремиума — когорты на дашборде и «Фремиум».
- A5 конверсия на дашборде = оплаты из визитов периода ÷ визиты, начавшиеся с дверей.
- A6 московские дни на дашборде; фильтр `|msk` и «(МСК)» в «Заказах», «Отзывах», «Визитах».
- A7 `track.js`: engaged = клик/клавиша, или прокрутка ≥25%, или 15 с видимого пребывания
  (touchstart и первый пиксель скролла убраны — половина «задержавшихся» длилась 2 с).
- A8 наблюдатель блоков: threshold 0 + rootMargin −40% снизу (блок «виден», когда его верх в
  верхних 60% экрана) — `sec_article` и высокие блоки лендинга теперь срабатывают.
- A10 «Визиты» пересобраны на `web_visits`: вход/выход, страницы, длительность, скролл, канал
  и кампания, устройство с шириной, гео, заказы; раскрытие — события визита; переключатель
  «настоящие / все».
- Возвраты: вебхук `refund.succeeded` → `orders.refunded_at/refund_kopecks`
  (подлинность — перезапросом платежа, как у оплаты). ⚠️ В ЛК ЮKassa нужно включить событие
  `refund.succeeded` для вебхука — иначе возвраты в выручке не отразятся.
- Сайдбар: Дашборд · Воронки (бывшая «Аналитика», без KPI/фремиума/ленты событий) · Визиты ·
  Действия · Заказы · Клиенты · Отзывы · … · Задачи (в конец).

**Проверка:** `step3_smoke.py` (36 проверок: страницы, период МСК, расход add/delete/валидация,
каналы email/ya.ru, наследование канала при возврате, path-scoped шаги, усечённая бесплатная
дверь) + `analytics_selftest.py` — зелёные; визуально на localhost (сетка, график, когорты, форма).
**Для заказчика:** включить `refund.succeeded` в ЛК ЮKassa; раз в неделю вносить расход
Директа/Meta на дашборде (без него цены = прочерк).


## 03.09.2026 — ШАГ 4 РЕВИЗИИ: конверсия, копия, SEO, вес + правки дашборда (V1.077)

**Дашборд по замечаниям заказчика:** (1) в «Воронках» таблица «Каналы» показывала 0 продаж
у рекламы, а «Источники» строкой ниже — 1 490 ₽: заказы без `visit_id` (до модели визитов
14.08 или без куки) в каналы не попадали. Теперь такие заказы относятся к каналу по UTM
самого заказа (`admin_funnels._order_channel_by_utm`). (2) Сверху дашборда — ряд общих кубиков
(визиты, задержались, действия-клики, анкеты фремиума, заказов создано, оплат, выручка,
конверсия), каждый с предыдущим периодом; подробности («Деньги подробнее», «Трафик подробнее»,
когорты, расход) — ниже.

**Конверсия (I1–I4, I9):** шапка на `/free/r/<token>` ведёт в `/order?free=` (не терялся
рисунок), в продающем блоке — «посмотреть пример отчёта»; на телефоне в шапке появился вход
«Бесплатно» (навигация <880px скрыта целиком); форма заказа: тема «Свободная тема» и дата
рисунка «сейчас» предзаполнены (`routes._order_defaults`); чекаут: «картой или через СБП…
после оплаты вы вернётесь на сайт» + пилюли способов оплаты; страница ожидания по ссылке
получила состояние «дольше обычного» (после ~90 с поллинга).

**Аналитика (I5):** образцы и отчёты на `/r/<token>` при отдаче получают Метрику + track.js
и `?v=` на css (`routes._report_page`) — самые просматриваемые страницы конверсии были не
измерены. `/admin/login` больше не грузит track.js.

**SEO/вес (I6–I7):** sitemap без фальшивого lastmod=сегодня (только у статей); H1 на `/blog`
и `/order`; свои description у `/order` и `/blog`; 404 — noindex; `strict_slashes=False`
(`/blog/` = `/blog`); ссылки на `/free/` без редиректа; убран 76-КБ favicon.svg; `fonts.css?v=`
на лендинге.

**Копия (I8):** дисклеймер писем, description по умолчанию, JSON-LD, CTA статей — в
формулировках философии 2.3 (не «навыки»); «оплата получена» обещает «в течение часа»;
«Голос Рисунка» → «Голос рисунка»; подсказки форм читаемым цветом (`--muted`, было 2.9:1);
aria-label у полей почты фремиума; пустой кабинет и вход упоминают бесплатный разбор;
устаревшая копия админки (ЮKassa «stub», «письма не отправляются») исправлена; в «Действиях»
подписи целей из реестра.

**Проверка:** `step4_smoke.py` (24 проверки) + step2/step3/feedback + `analytics_selftest.py` —
зелёные. Шаг 4 закрывает список ревизии 02.09; не сделано намеренно: восстановление входа
по `input type=month` и переименование продукта по всем страницам (отдельная задача копирайта).


## 03.09.2026 — БЛОГ: SEO-структура + 25 статей через Gemini (V1.078)

**Основание:** проверка органики и структуры блога (02.09). Органика — ~1 визит в день (19 за
20 дней, 16 с Яндекса), статьи 7–12 просмотров каждая, в основном свои. Структура: 14 коротких
(500–980 слов) статей без даты, автора, картинок, FAQ-разметки, без ссылки на бесплатный разбор,
четыре без входящих ссылок, title 66–92 символа, и — главное — блог утверждал «по рисунку нельзя
понять характер», противореча продукту (философия 2.3). Заказчик: «Gemini, расширить, кросс-ссылки,
SVG-миниатюры как сейчас, a..z».

**Шаг A — шаблоны/код (`app/blog.py`, `templates/blog_post.html`, `blog_index.html`,
`_blog_mid_cta.html`, `_blog_thumb.html`).** Frontmatter расширен: `updated`, `category`
(worry/age/reading/tests/activities/product), `related`, `seo_title`. Статья: хлебные крошки +
BreadcrumbList, дата/обновлено/время чтения/автор, призыв к бесплатному разбору ПОСЛЕ ВТОРОГО
раздела (маркер `MID_CTA_MARKER` вставляет `blog.py`, рендерит шаблон), финальная карточка с
главной кнопкой «Проверить свой рисунок бесплатно» (`/free/#name`) и второй — заказ, «Читайте
также» (3 карточки: related из frontmatter, добор по рубрике), Article JSON-LD с Person-автором,
image, publisher.logo, wordCount, FAQPage JSON-LD из раздела «Частые вопросы» (`**Вопрос?**` +
абзац), `article:published_time/modified_time`. Title без « — Голос рисунка». Хаб `/blog`:
вводный текст, навигация по рубрикам, группы карточек с датой и временем чтения,
CollectionPage/ItemList JSON-LD, канонический `/blog`. Подвал сайта получил «Блог».
Sitemap lastmod статей = `updated`. Цели: blogpost_free, blogpost_free_mid, blog_related_open,
blog_hub_free, blogpost_crumb_home.

**Шаги B/C — тексты (`scripts/blog_gen.py`, gemini-2.5-pro).** Системный промпт = философия 2.3
(чтение ребёнка как гипотеза с привязкой к детали, традиции и возвратом к ребёнку; HARD-баны
линтера отчётов; запрет отрицать продукт), обязательная структура (лид, «Коротко», H2-вопросы,
«Что типично по возрастам», «Что сделать сегодня», «Когда стоит показать специалисту», «Частые
вопросы», «Источники»), 4–7 внутренних ссылок только из разрешённого списка (в т.ч. обязательно
`/free/#name` и пример отчёта по возрасту). Линтер после генерации: HARD_BANNED из
`pipeline/lint.py`, длина, title ≤60/description ≤155, разделы, ссылки; автопоправки
(относительные ссылки, обрезка description), до 3 попыток с repair-замечаниями. 14 существующих
статей РАСШИРЕНЫ (исходник — материал; дата публикации сохранена; три перенацелены: «тёмные
цвета» → «значение цветов», «7 признаков» → чек-лист, «не диагноз» → «насколько точны тесты»;
«PDF-отчёт» — актуальные 7 направлений). 11 НОВЫХ: перестал рисовать; маленькие рисунки в углу;
закрашивает лица; что рисует ребёнок в 3 / 4 / 5 / 6–7 лет; тест «Рисунок семьи»; «Нарисуй
человека»; «Дом-дерево-человек» и «Несуществующее животное»; «анализ детского рисунка онлайн
бесплатно». У каждой новой — своя SVG-миниатюра в `_blog_thumb.html` (та же концепция дудла,
цвета через токены). Проверка: `scripts/blog_check.py` (длины, ссылки, сироты, противоречия).
Итог: 25 статей, 1447–2069 слов (в среднем ~1670), у всех FAQ-разметка, ссылка на бесплатный разбор и пример отчёта, сирот и битых ссылок нет.

**Правила на будущее:** новая статья = файл в `content/blog/` с полным frontmatter + ветка
миниатюры в `_blog_thumb.html`; тексты — через `scripts/blog_gen.py --slug` (добавить в PLAN);
после генерации — `scripts/blog_check.py`. Дата `updated` двигает статью вверх в хабе и sitemap.

## 08.09.2026 — ИНЦИДЕНТ: платный воркер «жив», но heartbeat 40 ч не обновлялся (UseCase #32)
**Симптом:** «Бета» → `worker (платные отчёты): 145605 c назад`, юнит `active (running)` 6 дней,
в worker.log тишина. **Диагноз на сервере:** WAL 649 МБ, основной файл БД не менялся с 07.09 01:00
(момент остановки heartbeat); strace — цикл крутится каждые 5 с, но ничего не пишет; /proc/locks —
процесс держит read-mark WAL постоянно. **Причина:** одноразовое «database is locked» в `_beat()`
проглочено `except: pass`, транзакция осталась открытой → снимок БД заморожен: записи падают
BUSY_SNAPSHOT, **SELECT очереди не увидел бы новых оплат**, чекпоинт WAL не проходит. Тот же
lock-ошибкой 04.09 07:00 упал free_worker (его поднял systemd — потому не завис).
**Сделано на проде (по разрешению заказчика):** `systemctl restart golosrisunka-worker`,
`wal_checkpoint(TRUNCATE)` → WAL 0 Б, оба heartbeat живые, очередь пуста (заказов в `paid` не было —
ничего не потеряно). **Код (локально, ещё не в проде):** `worker.py`/`free_worker.py` — heartbeat
с rollback + одно WARNING на инцидент, guard `conn.in_transaction` в начале итерации, `sqlite3.Error`
в цикле → rollback/пауза вместо смерти или зависания; `app/free_retention.py` — удаление файлов ДО
транзакции (одна `executemany`), чтобы не держать WAL-замок на время unlink. Проверено скриптом с
искусственной блокировкой (`BEGIN IMMEDIATE` во втором соединении): откат, один WARNING, восстановление.
**Не выяснено:** кто держал запись >5 с (оба раза — начало часа; в web-логе ничего долгого; вероятнее
всего — задержка fsync на общем диске Fornex). Защита теперь не зависит от причины.
**Дальше:** `.\release.bat` + `.\deploy-remote.bat` по команде заказчика (перезапуск юнитов внутри).
