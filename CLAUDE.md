...
- **ВСЕ цены/цифры — из админки «Цены» (`/admin/prices`, с 02.07)**: рантайм `data/products.json`
  поверх git-дефолта `config/products.json` (оверлей, UseCase #28). Хардкод цен в шаблонах/коде ЗАПРЕЩЁН.
  На сайте две цены: «до скидки» (зачёркнутая, `old_price_rub`, пусто = не показывается) и «к оплате»
  (`price_rub`); ЮKassa получает цену «к оплате» минус промокод (`orders.py`).
...
- **Файлы, которые пишет админка (products.json, report_texts.json, …) — ТОЛЬКО в `data/`**
  (на проде её владеет www-data; data/ в .gitignore → git pull не конфликтует). Писать из веб-процесса
  в дерево репо (config/ и т.п.) ЗАПРЕЩЕНО: на проде это root-owned git clone → 500 PermissionError
  (UseCase #28). `config/*.json` — read-only дефолты; читатели (`get_products()`/`get_report_texts()`)
  предпочитают data/-оверлей. Запись — атомарная (tmp + rename, `_atomic_write_json` в admin.py).
...
- **ФРЕМИУМ `/free` (см. память [[freemium-beta]])**: своя схема/промпт/линтер (`pipeline/free_*`),
  ...
  **ПОПРАВКА 14.08 (решение заказчика):** правило больше НЕ распространяется на глобальную
  шапку — там стоит пункт «Бесплатная проверка» → `/free-check`. В текстах лендинга правило
  действует как прежде.
- **ПОСАДОЧНАЯ `/free-check` (холодный платный трафик Meta/Instagram)**: `templates/free_check.html`,
  единственное действие — уйти в `/free/`. Ни цен, ни платного продукта, ни блога: любая вторая
  ссылка уводит оплаченный клик. В ОТЛИЧИЕ от `/free` — индексируется (в sitemap, решение
  заказчика). **Карточки тревог — ТЕКСТ, а не выбор** (deep-link `?concern=` был и убран
  14.08): список тревог живёт в ОДНОМ месте — `config/free_texts.CONCERNS`, и второй
  интерактивный список молча разошёлся бы с первым. В теле страницы ровно один адрес
  назначения — `/free/#name`. Шапка на этой странице
  подменяет платный призыв через `header_cta` (см. `_header.html`; у остальных страниц — дефолт).
  ⚠️ Класс `.reveal` на страницах вне `landing.html` НЕ использовать: он `opacity:0`, а
  наблюдатель живёт в инлайн-скрипте лендинга — блок останется невидимым навсегда.
...
- **Новый раздел админки «Цены» (`/admin/prices`)**: у каждого продукта «цена до скидки» (зачёркнутая)
  и «цена к оплате» + живой пересчёт % скидки; валидация (целые; до > после; пусто «до» = зачёркнутая
  не показывается, ключ удаляется). Лендинг/форма заказа/schema.org подхватывают без рестарта (mtime-кэш).
  Ценовые поля УБРАНЫ из «Настроек сайта» — одна страница-источник. Платёжная цепочка НЕ менялась:
  ЮKassa по-прежнему получает `order.price_kopecks` = цена «к оплате» × 100 минус промокод.
  Дефолтные цены: snapshot 2999 (до скидки 4900), development 4999 (до 6900).
- **Фикс прод-500 при сохранении из админки (V1.048, UseCase #28)**: хендлеры писали в `config/*.json` —
  на проде root-owned → `PermissionError`. Задело бы и «Настройки сайта»/«Тексты отчёта» (латентно с их
  создания). Теперь ВСЕ админ-правки пишутся атомарно в `data/products.json` / `data/report_texts.json`
  (оверлей поверх config-дефолтов); `get_products()`/`get_report_texts()` читают data/ при наличии.
  Задеплоено, заказчик проверил сохранение цен в проде — работает.
