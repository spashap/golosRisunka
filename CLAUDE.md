...
## Состояние на 03.09.2026 (вечер) — БЛОГ ПЕРЕСОБРАН: 25 статей, SEO-структура (V1.078, В ПРОДЕ)
Блог = SEO-канал в философии 2.3. `app/blog.py`: frontmatter `updated/category/related/seo_title`,
FAQ → FAQPage JSON-LD, призыв к бесплатному разбору после второго H2. Хаб `/blog` по рубрикам.
Тексты пишет/расширяет **`scripts/blog_gen.py`** (Gemini 2.5 Pro, PLAN внутри, линтер + автопоправки,
`--slug`/`--all --skip-done`), проверка — `scripts/blog_check.py`. Миниатюры — инлайн-SVG в
`_blog_thumb.html` (ветка на slug). Статей 25 (25 статей, 1447–2069 слов (в среднем ~1670), у всех FAQ-разметка, ссылка на бесплатный разбор и пример отчёта, сирот и битых ссылок нет). Журнал — DevelopmentStatus.md (03.09, блог).
**Правило:** статья не должна отрицать продукт («по рисунку нельзя понять характер» — запрещено);
каждая статья ссылается на `/free/#name` и на пример отчёта по возрасту.

## Состояние на 03.09.2026 — ШАГ 4 (конверсия/копия/SEO/вес) + дашборд с кубиками (V1.077, В ПРОДЕ)
Ревизия 02.09 закрыта целиком (шаги 2–4). Дашборд: сверху ряд общих кубиков, ниже детали;
«Каналы» в «Воронках» относят заказы без визита к каналу по UTM заказа. Конверсия: шапка на
разборе ведёт в заказ с рисунком, «Бесплатно» в мобильной шапке, форма заказа с
предзаполненными темой/датой, честная копия чекаута (СБП, возврат на сайт), «дольше обычного»
на странице ожидания. `/r/<token>` (образцы и отчёты) получают Метрику+track.js и `?v=` при
отдаче (`routes._report_page`). SEO: sitemap без фальшивого lastmod, H1/description на /blog и
/order, 404 noindex, `strict_slashes=False`. Копия — философия 2.3 везде (письма, meta, JSON-LD,
блог). Журнал — DevelopmentStatus.md (03.09).
**Дальше:** от заказчика — расход рекламы на дашборде раз в неделю, `refund.succeeded` в ЛК
ЮKassa, 14 целей Метрики, ящик поддержки; по коду — новых задач из ревизии нет.

## Состояние на 02.09.2026 (ночь) — ШАГ 3: НОВАЯ ГЛАВНАЯ АДМИНКИ + определения аналитики (V1.076, В ПРОДЕ)
`/admin` = дашборд пяти вопросов (`app/admin_dashboard.py`): проблемы → деньги (минус возвраты,
по каналам, vs предыдущий период) → настоящий трафик (график 30 дней) → когорты фремиума →
расход и цена лида/продажи (`ad_spend`, вводится руками). Дни — МОСКОВСКИЕ (`dash.period`,
фильтр `|msk`). Правила аналитики: возврат после паузы наследует канал (A1), канал `email` (A2),
шаги-наблюдения привязаны к странице `type@/path` (A3), бесплатная дверь в «Воронках» кончается
на загрузке (A4), engaged = клик/скролл≥25%/15 с (A7), секции — threshold 0 + rootMargin (A8),
«Визиты» на `web_visits` (A10). Возвраты — вебхук `refund.succeeded` (⚠️ включить в ЛК ЮKassa).
Бывшая «Аналитика» → «Воронки». Журнал — DevelopmentStatus.md (02.09, шаг 3).
**Дальше:** шаг 4 — конверсия/копия/SEO/перф (I1–I10 отчёта, память [[audit-2026-09-02]]).

## Состояние на 02.09.2026 (вечер) — РЕВИЗИЯ, ШАГ 2: критические фиксы (V1.074, В ПРОДЕ)
Полная ревизия сайта и админки (артефакт-отчёт, память [[audit-2026-09-02]]) → шаг 2 закрыт:
утечка email по номеру заказа (`orders.access_token` + кука `gr_o<id>`), настоящие
оферта/политика/согласие/контакты (`templates/legal_*.html`, реквизиты `settings.LEGAL`,
почта `settings.SUPPORT_EMAIL`), сессия после оплаты при опередившем вебхуке, второй рисунок
из фремиума, сканеры вне визитов (визит = только HTML-страница; `track.is_scanner_path`),
тестовые данные вне KPI (`is_test`, `TEST_EMAILS`, кука владельца `gr_ignore` через
`/admin/ignore-me`), безопасность (secure-куки, токен админки со сроком, троттлинг входа,
CSRF в админ-формах, `scripts/deploy/nginx_hardening.sh`), честные обещания фремиума
(повторная загрузка на том же токене, лимит сохраняет почту, черновик заказа живёт до успеха).
Журнал — DevelopmentStatus.md (02.09, шаг 2). **Правила:** любой URL с порядковым id обязан
проверять владение (UseCase #31); визит создаётся только успешной HTML-страницей (#30);
новые админ-формы — с `<input type="hidden" name="csrf" value="{{ csrf }}">`.
**Дальше:** шаг 3 (определения аналитики A1–A11 + новая главная админки), шаг 4 (конверсия/копия/SEO/перф).

## Состояние на 02.09.2026 — ОТЗЫВЫ (звёзды + текст) + GitHub-инцидент (V1.072, В ПРОДЕ)
- **Оценка результата родителем**: таблица `feedback` (одна на продукт, upsert), блюпринт
  `app/feedback.py`, один виджет `_feedback_widget.html`+`feedback.js`+`.rate` на бесплатный
  разбор (`/free/r/`) и платный отчёт (`/r/` — дописывается при отдаче перед `</body>`, образцы
  без виджета). Кабинет показывает оценку/ссылку «оценить»; письма приглашают оценить.
  Админка: «Клиенты» → колонки «Получил» (ссылки на отчёты и разборы) и «Отзыв»; «Заказы» →
  «Оценка»; **раздел «Отзывы»** `/admin/feedback` (средний балл, отклик, распределение —
  ОТДЕЛЬНО по двум продуктам). Журнал — DevelopmentStatus.md (02.09).
- **⚠️ GitHub-аккаунт `spashap` скрыт с 28.08.2026** (память [[github-account-flagged]],
  правила — `C:\projects\CosmyDay\CLAUDE.md` §1e). Локальный push/pull по SSH работает;
  **прод НЕ может делать `git pull`** (анонимный HTTPS → 404, deploy-ключи тоже не читают).
  Временный деплой: `git push fornex main:refs/heads/claude-deploy` → на сервере
  `git merge --ff-only claude-deploy && git branch -D claude-deploy && SKIP_PULL=1 ./deploy.sh`.
  `deploy-remote.bat` в этом режиме НЕ подходит (он зовёт `./deploy.sh` без `SKIP_PULL`).
  Никаких GitHub-токенов на сервере. После реинстейта — обычный `./deploy.sh`.

## Состояние на 14.08.2026 — АНАЛИТИКА ПЕРЕСОБРАНА + ПОСАДОЧНАЯ ПОД РЕКЛАМУ (V1.065→V1.070, В ПРОДЕ)
Три вещи за день, все задеплоены. Подробности — журнал `DevelopmentStatus.md` (14.08),
память [[analytics-rebuild]] и [[freemium-beta]].
- **Аналитика фремиума** (V1.065–V1.066): блок на дашборде (прошли анкету / только email /
  запросили разбор) + раздел **«Фремиум»** (`/admin/free-analytics`); фремиум-родители видны
  в «Клиентах» (вкладки) и в «Заказах» (вкладка «Бесплатные разборы»).
- **ПЕРЕСБОРКА ИЗМЕРЕНИЙ** (V1.068). Старая аналитика не отвечала ни на один вопрос: не было
  ВИЗИТА (считались уникальные посетители за период), «воронка» делила девять независимых
  множеств, у событий не было адреса страницы, блог не измерялся вообще, глубины прокрутки не
  существовало, UTM только первого касания, отказы не фиксировались, а полсотни `reachGoal`
  уходили в пустоту (цели не заведены в счётчике). Теперь: `web_visits` + `events.visit_id/path`
  + `orders.visit_id`; свой `static/js/track.js` (вне гейта Метрики); глубина прокрутки и
  видимость блоков; события всех отказов; воронки по визитам (см. правило ниже);
  реестр целей `config/goals.py` + `scripts/metrika_goals_sync.py`; самопроверка
  `scripts/analytics_selftest.py`. Раздел **«Задачи»** (`/admin/todo`) — то, что делается руками.
- **ПОСАДОЧНАЯ `/free-check`** (V1.069–V1.070) под холодный трафик Meta/Instagram — см. правило
  выше. Третья дверь в воронках сравнивает «реклама → посадочная» с «реклама → главная».
- **⏳ ЧЕГО ЖДЁМ / что нужно СДЕЛАТЬ ЗАКАЗЧИКУ (в порядке важности):**
  1. **Завести 14 целей в Метрике руками** (~10 мин) — задача уже стоит в `/admin/todo` с полной
     инструкцией; список целиком — `scripts/metrika_goals_sync.py --manual` →
     `data/metrika_goals_manual.txt`. Без этого Метрика не считает конверсии и автостратегиям
     Директа не на чем учиться (собственная аналитика при этом работает).
  2. **Проверить `/free-check` на телефоне** (переход по кнопке в мастер) и шапку на ноутбуке
     шириной ~900px (пятый пункт меню — новый).
  3. **Просьба подрядчику по Директу** (текст готов в `projectSpec/yandex-api-token-howto.md`):
     разметить объявления UTM (`utm_source/medium/campaign/content/term`) и связать рекламный
     аккаунт со счётчиком 109824945. Сейчас объявления несут только `yclid` — видно «это Директ»,
     но не какая кампания; расход не виден вообще.
  4. **Для рекламы Meta/Instagram UTM обязателен**: у Meta нет аналога yclid, без меток трафик
     ляжет как `referral`/`direct` без кампании.
- **Решено НЕ делать:** API Вебмастера и Метрики (кроме заведения целей) не подключаем — Директ
  ведёт подрядчик, доступа нет; данные Вебмастера целиком видны в его интерфейсе. `app/metrika.py`,
  `app/webmaster.py`, `scripts/yandex_token_check.py` лежат в репозитории мёртвыми, токен не нужен.
...
- ~~Прод на stub-оплате~~ — УСТАРЕЛО: ЮKassa живёт в бою с 23.06, оплата настоящая (не
  перепроверять на обычных деплоях). После правок шаблонов/CSS: на сервере `./deploy.sh`
  (nginx/TLS он НЕ трогает). Если когда-нибудь снова включат Cloudflare orange —
  «Block AI bots»/Bot Fight Mode держать ВЫКЛ + purge (UseCase #18); при grey-cloud неактуально.
